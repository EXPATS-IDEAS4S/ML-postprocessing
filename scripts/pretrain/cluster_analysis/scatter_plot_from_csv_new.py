...
# === NEW CONFIG ENTRIES ===
PLOT_ALL_POINTS = True       # plot all scatter points (small & transparent)
PLOT_MEAN_POINTS = True      # plot the big mean markers
PLOT_ALL_LABELS_TOGETHER = False
PLOT_SEPARATE_LABELS = True

RAW_PT_SIZE = 25             # size of individual points
RAW_PT_ALPHA = 0.25          # transparency of raw points
MEAN_PT_SIZE = 160           # size of mean markers
...
def extract_all_points(df, var_name, var_type):
    """Return dataframe containing all raw points (per label) for scatter plotting."""
    subset = df[df["var"] == var_name].copy()
    ...
    if var_type == "percentile":
        values = subset[["label"] + PERCENTILE_COLS].copy()
        for p in PERCENTILE_COLS:
            values[p] = values[p] * mult + offset
    else:
        subset[CATEGORICAL_COL] = pd.to_numeric(subset[CATEGORICAL_COL], errors="coerce")
        values = subset[["label", CATEGORICAL_COL]].copy()
        values[CATEGORICAL_COL] = values[CATEGORICAL_COL] * mult + offset
...
raw_x = extract_all_points(df, VAR_X, 
                           "percentile" if VAR_X in PERCENTILE_VARS else "categorical")

raw_y = extract_all_points(df, VAR_Y, 
                           "percentile" if VAR_Y in PERCENTILE_VARS else "categorical")
print(f"Extracted raw data for VAR_X='{VAR_X}' and VAR_Y='{VAR_Y}'")
...
def plot_scatter(mean_x, mean_y, var_x, var_y, output_path, separate_label=None, transparent=False):
    ...
    for lbl in labels_common:
        ...
        # === RAW POINTS ===
        if PLOT_ALL_POINTS:
            x_raw_vals = raw_x[raw_x["label"] == lbl]
            y_raw_vals = raw_y[raw_y["label"] == lbl]

            # Expand to raw scatter points (pair percentile-raw combinations)
            for _, rx in x_raw_vals.iterrows():
                for _, ry in y_raw_vals.iterrows():
                    if var_x in PERCENTILE_VARS:
                        xs = [rx[p] for p in PERCENTILE_COLS]
                    else:
                        xs = [rx[CATEGORICAL_COL]]

                    if var_y in PERCENTILE_VARS:
                        ys = [ry[p] for p in PERCENTILE_COLS]
                    else:
                        ys = [ry[CATEGORICAL_COL]]
        ...
        if PLOT_MEAN_POINTS:
            ...
            for p_x, x_v in x_vals.items():
                for p_y, y_v in y_vals.items():
                    p_marker = MARKERS.get(
                        p_y if var_y in PERCENTILE_VARS else p_x,
                        "o"
                    )
                    ax.scatter(
                        x_v,
...
if PLOT_ALL_LABELS_TOGETHER:
    main_out = os.path.join(path_to_dir, f"scatter_{VAR_X}_vs_{VAR_Y}_all.png")
    plot_scatter(mean_x, mean_y, VAR_X, VAR_Y, main_out, transparent=False)
    print(f"💾 Saved full scatterplot: {main_out}")

# === PER LABEL ===
if PLOT_SEPARATE_LABELS:
    for lbl in sorted(set(mean_x["label"]).intersection(mean_y["label"])):        
        lbl_str = str(int(lbl))
        lbl_out = os.path.join(path_to_dir, f"scatter_{VAR_X}_vs_{VAR_Y}_label_{lbl_str}.png")
