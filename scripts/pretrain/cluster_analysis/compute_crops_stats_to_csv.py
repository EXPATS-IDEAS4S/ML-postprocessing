"""
Compute crop-level statistics for CMSAF and IMERG datasets.
...
The input dataset is selected with `--mode`:

- `train`: process training crops from the feature CSV generated by the
  training feature-preparation pipeline.
  
- `test`: process test/case-study crops. Test mode first looks for the
  feature CSV
  `/sat_data/fig/<run_name>/test/features_test_dataset_ESSL_<run_name>.csv`
  (or `features_preparation.test_feature_csv_path` if configured) and uses
  its crop paths, labels, distances, and timestamps. If that CSV is missing,
  it falls back to recursively discovering crop files under
  `features_preparation.crops_test_path`.

Main steps:
-----------
1. Load the crop list and labels:
...
Inputs:
-------
- Train feature CSV with crop paths and labels.
- Test feature CSV with crop paths, timestamps, labels, distances, and
  features, when running `--mode test`.
...
- `features_preparation.test_feature_csv_path` : str, optional
      ...
      to recursive file discovery.
- `features_preparation.test_crop_type` : str, optional
      Restrict fallback recursive discovery to directories named e.g.
...
- The current GRL test CSV stores `datetime` as a stringified pandas Index;
  chunking therefore falls back to the date prefix in the crop filename when
  needed.


How to run for benchmarking and debugging:
conda run -n vissl python scripts/pretrain/cluster_analysis/compute_crops_stats_to_csv.py --mode test --benchmark-rows 2000

How to run on the training dataset:
conda run -n vissl python scripts/pretrain/cluster_analysis/compute_crops_stats_to_csv.py --mode train
...
- Run the script using:
    conda run -n vissl python scripts/pretrain/cluster_analysis/compute_crops_stats_to_csv.py --mode test
...
def iter_label_chunks(csv_filename, chunk_size, benchmark_rows=None):
    ...
    if "datetime" in df_labels.columns:
        datetime_days = df_labels["datetime"].astype(str).str.slice(0, 8)
        path_days = df_labels["path"].map(lambda path: os.path.basename(path)[:8])
        df_labels["__chunk_day"] = datetime_days.where(
            datetime_days.str.match(r"^\d{8}$"),
            path_days,
        )
...
def count_csv_rows(csv_filename):
    """Count CSV data rows without loading feature vectors into memory."""
    with open(csv_filename, "r", encoding="utf-8") as csv_file:
        return max(sum(1 for _ in csv_file) - 1, 0)


def resolve_test_csv_path(features_config, run_name):
    """Return the configured or conventional test feature CSV path if it exists."""
    configured_path = features_config.get("test_feature_csv_path")
    if configured_path:
        return configured_path

    candidate_paths = []
    output_path = features_config.get("output_path")
    if output_path:
        candidate_paths.append(
            os.path.join(output_path, "test", f"features_test_dataset_ESSL_{run_name}.csv")
        )

    fig_output_path = features_config.get("fig_output_path")
    if fig_output_path:
        candidate_paths.append(
            os.path.join(fig_output_path, "test", f"features_test_dataset_ESSL_{run_name}.csv")
        )

    candidate_paths.append(
        os.path.join("/sat_data/fig", run_name, "test", f"features_test_dataset_ESSL_{run_name}.csv")
    )
...
def list_crop_files(crops_path, extension, recursive=False, crop_type=None):
    """Return sorted crop files from a flat training folder or nested test tree."""
    extension = extension.lstrip(".")
    crop_files = []

    if recursive:
        for root, _, filenames in os.walk(crops_path):
            if crop_type is not None and os.path.basename(root) != crop_type:
                continue
            for filename in filenames:
                if filename.endswith(f".{extension}"):
                    crop_files.append(os.path.join(root, filename))
    else:
        crop_files = [
            os.path.join(crops_path, filename)
...
def iter_crop_file_chunks(crop_files, chunk_size, benchmark_rows=None):
    """Yield crop-file rows in bounded chunks without splitting crop days."""
    if benchmark_rows is not None:
        crop_files = crop_files[:benchmark_rows]

    df_labels = pd.DataFrame({"path": crop_files, "label": np.nan})
    df_labels["__chunk_day"] = df_labels["path"].map(
        lambda path: os.path.basename(path)[:8].replace("-", "")
    )
    df_labels = df_labels.sort_values(["__chunk_day", "path"])
...
def get_crop_metadata(row, config, image_crops_path):
    ...
    crop_path = str(row["path"])
    crop_filename = os.path.basename(crop_path)
    crop_dir = os.path.dirname(crop_path) if os.path.dirname(crop_path) else image_crops_path
...
def main(
    ...
    # if we process training data then we read the paths from the training section of the config
    if mode == "train":
        # crops path
        ...
        crops_path = features_config['crops_test_path']  # f"{data_base_path}/{crops_name}_test/{data_format}/"
        image_crops_path = features_config['images_test_path']      #f"{data_base_path}/{crops_name}_test/{data_format}/1/"
    else:
    ...
    test_input_source = None
    test_crop_type = features_config.get("test_crop_type")
    if mode == "test":
        test_csv_filename = resolve_test_csv_path(features_config, run_name)
        if test_csv_filename is not None:
            crop_files = None
            test_input_source = "csv"
            n_samples = count_csv_rows(test_csv_filename)
            logger.info("Using test feature CSV: %s", test_csv_filename)
        else:
            crop_files = list_crop_files(
                crops_path,
                data_format,
                recursive=True,
                crop_type=test_crop_type,
            )
            test_input_source = "recursive_files"
            n_samples = len(crop_files)
            logger.info(
                "Discovered test crops recursively under %s%s.",
                crops_path,
                f" for crop type '{test_crop_type}'" if test_crop_type else "",
            )
    else:
        crop_files = None
    ...
    processed_chunks = 0
    if mode == "test":
        if test_input_source == "csv":
            chunk_iter = iter_label_chunks(
                test_csv_filename,
                crop_chunk_size,
                benchmark_rows=benchmark_rows,
            )
        else:
            chunk_iter = iter_crop_file_chunks(
                crop_files,
                crop_chunk_size,
                benchmark_rows=benchmark_rows,
            )
    ...
    for chunk_idx, df_labels in enumerate(chunk_iter, 1):
