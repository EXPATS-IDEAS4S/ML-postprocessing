...
How to run for benchmarking and debugging:
conda run -n vissl python scripts/pretrain/cluster_analysis/compute_crops_stats_to_csv.py --benchmark-rows 2000
...
def build_block_context(ds_day, ds_day_cma, crop_requests):
    ...
    # Detect variable type from the incoming DataArray name.
    is_lightning = ds_day.name == "euclid_msg_grid"
    is_precip = ds_day.name == "precipitation"
    ds_day_proc = ds_day
...
def get_time_windows(times, var=None, filter_imerg=False):
    """
    Group a list of crop timestamps by day.
    ...
    filter_imerg : bool, optional
        If True, precipitation timestamps are kept on the native 15-minute crop
        grid and later aligned to 30-minute IMERG fields in build_block_context.
    var : str, optional
        Variable name.
...
def compute_statistics(values_append, stats, var, mode, times, crop_filename, label, coords):
    ...
    if mode == "aggregated":
        ...
        for frame_dict in values_append:
            ...
            for stat in stats:
                stat_values = frame_dict["values"]
                if stat == "prec_fraction" and var == "precipitation":
                    stat_values = frame_dict.get("raw_values", frame_dict["values"])
                row[stat] = compute_single_stat(stat_values, stat, var)
            rows.append(row)
...
def compute_single_stat(values, stat, var):
    ...
    if stat == "None":
        ...
        # Each 30-minute precipitation rate is assigned to two 15-minute crop frames,
        # so the per-frame accumulation must be computed over 15 minutes.
        delta_t = 0.25  # hours
        return np.nansum(values) * delta_t
    elif stat == "prec_fraction" and var == "precipitation":
        total_pixels = len(values)
        prec_pixels = np.sum(values > 0.1)
