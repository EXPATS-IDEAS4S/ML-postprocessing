...
for col, time_group in enumerate(time_groups):
    ...
    # -----------------------------------
    # BARPLOT: counts by class (no dominance split)
    # -----------------------------------
    ...
    for l, label in enumerate(interesting_labels):
        ...
        # total counts for this class
        df_lbl = df_time[df_time["label"] == label]
        count = len(df_lbl)

        # plot single bar per class
        ax_count.bar(
            l,
            count,
            color=cls_color,
...
plt.savefig(
    csv_path + "/var_distributions_by_time_groups_no_dominance.png",
    dpi=150,
    bbox_inches="tight"
)
