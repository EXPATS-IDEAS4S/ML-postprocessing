...
from from_buckets.aux_functions_from_buckets import get_variable_info

def plot_dataset_maps(ds, ds_oro, output_path, label, cmap, vmin_vmax_df, hour):
    """
    ...
    for var in ds.data_vars:
        ...
        if percentile:
            title_text = f"{var_info['long_name']} {percentile}th percentile map for class {label} and hour {hour}"
...
for label in labels:
    for hour in hours:
        # Open dataset
        ds = xr.open_dataset(f"{output_path}{label}/percentile_maps_res_{n_div}x{n_div}_label_{label}_hour_{hour}.nc", decode_times=False, engine="h5netcdf")
        print(ds)

        # Plot and save maps
        path_figures = f"{output_path}{label}/"
        os.makedirs(path_figures, exist_ok=True)
        plot_dataset_maps(ds, ds_dem, path_figures, label, cmap, vmin_vmax, hour)
