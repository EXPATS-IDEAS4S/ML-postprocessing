...
from from_buckets.aux_functions_from_buckets import (
    extract_datetime,
    compute_categorical_values,
    find_crops_in_range
)
from from_buckets.get_data_from_buckets import read_file, Initialize_s3_client
from from_buckets.credentials_buckets import S3_ACCESS_KEY, S3_SECRET_ACCESS_KEY, S3_ENDPOINT_URL
...
for label in unique_labels:
    ...
    for hour in range(24):
        ...
        df_filtered.loc[:, 'datetime'] = df_filtered['path'].apply(safe_parse_datetime)

        # Drop rows where datetime parsing failed
        df_filtered = df_filtered.dropna(subset=['datetime'])

        # Filter by current hour
        df_filtered_hour = df_filtered[df_filtered['datetime'].dt.hour == hour]
        ...
        #         "{year:04d}-{month:02d}-{day:02d}T{hour:02d}:{minute:02d}:00".format(**extract_datetime(os.path.basename(p)))
        #     )
        # )
        ...
        results_list = Parallel(n_jobs=num_cores)(
            delayed(process_lat_lon)(i, j, lat_edges[i], lat_edges[i+1], lon_edges[j], lon_edges[j+1], df_filtered_hour, label)
        ...
        #Add metadata
        ds.attrs["description"] = f"Processed data for label {label} and hour {hour}"
        ...
        # # Save dataset
        out_dir = f"{output_path}percentile_maps/{label}/"
        os.makedirs(out_dir, exist_ok=True)
        output_filename = f"{out_dir}percentile_maps_res_{n_div}x{n_div}_label_{label}_hour_{hour}.nc"
